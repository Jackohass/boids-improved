...
typedef struct NeighbourWatchlistEntry {
	int index;
	float dist;
} NeighbourWatchlistEntry;
...
kernel void simulateBoid(
	__global Boid* boids,
	__global NeighbourWatchlistEntry* neighs,
	__global int* neighOffset,
	__global int* neighSize,
	...
	for (int neighIndex = 0; neighIndex < numNeighs; neighIndex++) {
		...
		Boid neighbour = boids[entry.index];
